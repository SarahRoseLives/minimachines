...
void EditorApp::render(SDL_Renderer* renderer) {
    ...
    if (m_showProperties)
        m_properties.draw(m_map, m_showNewMapDialog);
    m_properties.drawNewMapDialog(m_map, m_showNewMapDialog);
...
void EditorApp::endMapWindow() {
    ...
    if (ImGui::IsItemHovered()) {
        ...
        if (ImGui::IsMouseClicked(ImGuiMouseButton_Left))
            m_tools.onMouseButton(m_camera, m_map, m_toolState, localX, localY, true, true);
        if (ImGui::IsMouseReleased(ImGuiMouseButton_Left))
            m_tools.onMouseButton(m_camera, m_map, m_toolState, localX, localY, false, true);
...
void EditorApp::drawMenuBar() {
    if (ImGui::BeginMainMenuBar()) {
        if (ImGui::BeginMenu("File")) {
            if (ImGui::MenuItem("New")) m_showNewMapDialog = true;
        ...
        ImGui::Text("Tool: %s | Layer: %s", toolName, layerName);
